...
private fun GOMOKU_USER_STATS_URL(id: Int) =
    "$GOMOKU_API_URL/stats/users/$id"
...
class StatsServiceImpl(
    ...
    private fun getUserStatsRequest(id: Int) =
        Request.Builder().buildRequest(
            url = URL(userStatsRequestUrlBuilder(id)),
            method = HttpMethod.GET
        )

    override suspend fun getUserStats(id: Int): UserStatsOutputModel {
        val sirenUserStats =
            requestHandler<UserStatsOutputModel>(
                getUserStatsRequest(id)
            )
        return sirenUserStats.properties
    }
}
